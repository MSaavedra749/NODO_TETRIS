...
float pixel_a_porc(int pixel, int tam){
    return ((float)pixel / tam) * 100.0f;
}
...
void dibujar_mino(uint8_t tab_x, uint8_t tab_y, uint8_t col, bool dither)
{

    //Posicion del mino en pantalla
    int casillero_x = pantalla->tab_offsetX + tab_x * pantalla->mino_tamanio;
    int casillero_y = pantalla->tab_offsetY + tab_y * pantalla->mino_tamanio;

    int escala = pantalla->mino_tamanio /  TAMANIO_MINO;
    if(escala < 1) escala = 1;

    int tamanio_dibujo_real = TAMANIO_MINO * escala;

    int centro_x = (pantalla->mino_tamanio - tamanio_dibujo_real) / 2;
    int centro_y = (pantalla->mino_tamanio - tamanio_dibujo_real) / 2;

    int offset_x = casillero_x + centro_x;
    int offset_y = casillero_y + centro_y;
...
void dibujar_mino_pantalla(int pos_x, int pos_y, int col)
{
    int escala = pantalla->mino_tamanio / TAMANIO_MINO;
    for(int i= 0; i < TAMANIO_MINO; i++)
    {
        for(int j=0; j < TAMANIO_MINO; j++)
        {
            ...
            int inicio_x = pos_x + j* escala;
            int inicio_y = pos_y + i * escala;
            for(int x = inicio_x; x < inicio_x + escala; x++)
            {
                for(int y = inicio_y; y < inicio_y + escala; y++)
                {
...
int calcular_ancho_letra(uint8_t letra[8][8])
{
    ...
    return -1; /// le puse este return porque el codeblock me estaba puteando, le puse -1 por si es una error(??????
}
...
void dibujar_filtro_dither(uint8_t col)
{
    for(int x = 0; x < pantalla->ancho; x++){
        for(int y = 0; y < pantalla->alto; y++){
            if((x%2) == (y%2)){

...
#define TABLERO_CLASICO_COLS 10
...
typedef struct{
    ...
    bool modo_dx;
    int pieza_anterior;
    int textura_siguiente;
    int textura_anterior;
    int ancho_tablero;
    ...
    uint32_t record_puntos;

    bool animacion_borrar;
    int frames_animacion;
    int lineas_borradas;
    bool filas_marcadas[TABLERO_FILAS];
}Tetris;

//Por el momento lo creo aca
typedef struct
{
    uint8_t matriz_tablero[TABLERO_FILAS][16];
    int ancho_tablero;
    Pieza Pieza;
    float velocidad_ms;
    int cant_piezas;
